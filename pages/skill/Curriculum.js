...
const Curriculum = () => {
  ...
  const classCurriculum = curriculum[classKey];

  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={() => navigation.goBack()} style={styles.header}>
    <Ionicons name="arrow-back-outline" size={24} color="#FFFFFF" />
    <Text style={styles.headerText}>{classCurriculum.title}</Text>
  </TouchableOpacity>
          ...
          <View key={index} style={styles.chapter}>
            ...
            {chapter.topics.map((topic, subIndex) => (
              <View key={subIndex} style={styles.topic}>
                <Ionicons name="caret-forward-circle-outline" size={34} color="#f69722" style={styles.icon} />
                <View style={styles.topicTextContainer}>
                  <Text style={styles.topicTitle}>{index + 1}.{subIndex + 1} {topic.chapter}</Text>
